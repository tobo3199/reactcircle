...
function App() {
  ...
  */
  //const ranCordinates = Array.from(cordinates);
  const ranCordinates = [];
  const ranCordinates2 = [];
  const newRanCordinates = [];
  let value = 0;
  let value2 = 0;

  for (let i = 0; i < 14; i++) {
    ranCordinates[i] = Math.floor(Math.random() * 1000);
  }
  //console.log("ARRAY 1:");
  //console.log(ranCordinates);

  for (let i = 0; i < 14; i++) {
    ranCordinates2[i] = Math.floor(Math.random() * 1000);
  }
  //console.log("ARRAY 2:");
  //console.log(ranCordinates2);

  for (let k = 0; k < 14; k++) {
    value = ranCordinates[k];
    value2 = ranCordinates2[k];
    newRanCordinates.push([value, value2]);
  }
  //console.log("NEW COORDINATES:");
  //console.log(newRanCordinates);
  ...
  */
  ...
  const staticCordinates = Array.from(newRanCordinates);
  const cordinates = Array.from(staticCordinates);
    ...
    for (x; x <= 11; x++) {
      for (y = x + 1; y <= 12; y++) {
        for (z = y + 1; z <= 13; z++) {
          counter = counter + 1;
          console.log(
            "x: " + x + "y: " + y + "z: " + z + "counter: " + counter
          );

          const a =
            (cordinates[y][1] * cordinates[y][1] -
              cordinates[z][1] * cordinates[z][1] +
              cordinates[y][0] * cordinates[y][0] -
              cordinates[z][0] * cordinates[z][0]) /
            (2 * (cordinates[z][1] - cordinates[y][1]));
          const b =
            (cordinates[x][1] * cordinates[x][1] -
              cordinates[y][1] * cordinates[y][1] +
              cordinates[x][0] * cordinates[x][0] -
              cordinates[y][0] * cordinates[y][0]) /
            (2 * (cordinates[y][1] - cordinates[x][1]));
          const c =
            (cordinates[y][0] - cordinates[x][0]) /
            (cordinates[y][1] - cordinates[x][1]);
          const d =
            (cordinates[z][0] - cordinates[y][0]) /
            (cordinates[z][1] - cordinates[y][1]);

          x4 = (a - b) / (c - d);
          y4 = -d * x4 - a;

          radius1 = Math.sqrt(
            (cordinates[x][0] - x4) * (cordinates[x][0] - x4) +
              (cordinates[x][1] - y4) * (cordinates[x][1] - y4)
          );
